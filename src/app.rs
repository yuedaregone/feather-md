...
/// Custom event for file watcher notifications
#[derive(Debug, Clone)]
pub enum AppEvent {
    FileChanged,
}
...
impl App {
    ...
    pub fn run(&mut self, file_path: Option<PathBuf>) {
        ...
        // Determine the directory for local file access
        let file_dir: Arc<Option<PathBuf>> = Arc::new(
            file_path
                .as_ref()
        ...
        // Setup file watcher
        let _file_watcher: Arc<Mutex<Option<FileWatcher>>> = Arc::new(Mutex::new(
        ...
        let webview_builder = WebViewBuilder::new()
            ...
            .with_ipc_handler({
                ...
                let file_watcher = _file_watcher.clone();
            ...
            .with_custom_protocol("feather".into(), {
                ...
                move |_webview_id, request| {
                    handle_custom_protocol(request, &file_dir).map(Into::into)
                }
            });
        ...
        let theme_script = format!(
            ...
            serde_json::to_string(&self.config.theme).unwrap_or_default()
        );
        let _ = webview.evaluate_script(&theme_script);

        // Register file association on first run (Windows only)
            ...
            *control_flow = ControlFlow::Wait;
            ...
            match event {
                Event::UserEvent(AppEvent::FileChanged) => {
                    let file_path = current_file.lock().unwrap();
                    if let Some(ref path) = *file_path {
                        if let Ok(content) = read_file_with_encoding(path) {
                            let script = format!(
...
window.__feather_ipc = {
    postMessage: function(msg) {
        window.__feather_external_ipc(msg);
    }
};
"#;

/// Handle IPC messages from the frontend
fn handle_ipc(
    message: &str,
    _current_file: &Arc<Mutex<Option<PathBuf>>>,
    _file_watcher: &Arc<Mutex<Option<FileWatcher>>>,
    _event_loop_proxy: &tao::event_loop::EventLoopProxy<AppEvent>,
) {
    ...
    match msg_type {
        ...
        "reload" => {
            // File reload is handled via FileChanged event from watcher
        }
        "file-opened" => {
            if let Some(_path) = msg["path"].as_str() {
                // File path updated by frontend
            }
        }
        "exit" => {}
        _ => {}
...
/// Handle custom feather:// protocol for serving embedded assets and local files
fn handle_custom_protocol(
    request: wry::http::Request<Vec<u8>>,
    file_dir: &Arc<Option<PathBuf>>,
